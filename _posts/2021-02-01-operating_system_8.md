...
프로세스마다 기준(base), 상한(limit) 레지스터에 메모리 주소의 기준과 상한를 둔다. 
...
컴파일, 적재 시 바인딩의 경우에는 논리주소와 물리주소가 같다. 그러나 실행 시간 바인딩에서는 둘이 다르다. 
이런 경우 논리 주소를 가상 주소(virtual address)라 한다. 

프로그램 실행 중에는 가상 주소를 물리 주소로 바꿔줘야 한다. 이는 하드웨어인 메모리 관리기(MMU, Memory Management Unity)가 실행한다. 
또한 기준 레지스터를 재배치(relocation) 레지스터라고 부른다. 메모리 주소로 프로세스가 생성한 논리 주소가 보내질 때, 재배치 레지스터의 값을 더해 물리 주소로 만든다. 

### 1.4. 동적 적재(Dynamic Loading)
동적 적재에서는 프로세스 실행을 위해 프로세스 전체가 메모리에 올라와 있을 필요가 없다. 
프로세스의 각 루틴은 호출 전까지 재배치 가능한 상태로 디스크에서 대기한다. 
한 루틴이 다른 루틴을 호출할 경우, 먼저 호출하는 루틴이 호출된 루틴이 메모리에 있는지 확인한다. 
없다면 재배치가능 링커(relocatable linking loader)가 요구된 루틴을 메모리에 올리고 프로그램의 주소 테이블을 갱신한다. 
...
### 1.5. 동적 연결 및 공유 라이브러리(Dynamic Linking & Shared Libraries)
...
스텁이 실행될 때 자신을 루틴의 번지로 대체하는데, 라이브러리가 메모리에 없으면 디스크에서 가져온다. 
10개의 프로세스가 prinf와 같은 라이브러리를 사용한다고 하더라도 라이브러리 코드는 메모리에 한번만 올라간다. 
...
TLB는 매우 빠른 연관 메모리(associative memory)로 구성된다. TLB의 각 항목은 키(key)와 값(value)로 구성된다. 
...
따라서 여러 프로세서들이 동시에 같은 코드를 수행할 수 있다. 
...
### 6.1. 계측적 페이징(Hierarchical Paging)
...
각 항목이 4byte 라면 4MB 공간이 필요하게 된다. 이런 경우 모든 페이지 테이블을 주 메모리에서 연속적으로 할당하기를 고집하지 않을 것이다. 

한 방법은 2단계 페이징 기법(two-level pagin scheme)으로서 페이지 테이블 자체가 다시 페이지화 되는 것이다. 
이 경우 논리 주소는 <바깥 페이지 번호, 안쪽 페이지 번호, 페이지 변위>가 된다. 
이러한 방법으로 페이지 테이블을 나눔으로써, 운영체제는 프로세스가 이 분할들을 실제로 필요할 때까지 사용하지 않은 채로 남겨둘 수 있다. 

하지만 64비트 주소 공간에서는 여전히 페이지 테이블 크기가 크다. 
7단계 페이징 기법을 사용할 수도 있지만, 메모리 접근이 너무 많아지기 때문에 쓰지 않는다. 

### 6.2. 해시 페이지 테이블(Hashed Page Tables)
...
### 6.3. 역 페이지 테이블(Inverted Page Table)
