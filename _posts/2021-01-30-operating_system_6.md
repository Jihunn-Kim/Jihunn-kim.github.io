...
한 프로세스가 임계구역에 있는 동안 다른 프로세스들은 lock 획득 함수를 계속 실행하게 된다. 이를 바쁜 대기(busy waiting) 혹은 spinlock 이라 한다. 
프로세스들이 짧은 시간 동안만 임계구역에 있는다면 이런 방식도 유용하지만, 오랜 시간 다른 작업을 못할 수도 있다. 
...
### 6.3. 교착 상태와 기아(Deadlock and Starvation)
...
기아 상태는 프로세스들이 무한정 대기하는 것이다. 세마포와 연관된 큐가 프로세스들을 후입 선출(last-in, first-out, LIFO)순서로 꺼내온다면 발생할 수 있다. 
...
이는, 높은 우선순위 프로세스가 필요로 하는 자원을 사용하는 모든 프로세스들에 잠시 높은 우선순위를 주는 것이다. 
...
모니터 내부 작업중 잠시 프로세스가 대기하는 next queue.

8.2. 모니터를 사용한 식사하는 철학자 해결안은 넘어간다. 

### 8.3. 세마포를 이용한 모니터의 구현
각 모니터에는 mutex라는 세마포가 있으며 초기 값은 1이다. 
프로세스는 모니터에 들어가기 전 wait(mutex)를 실행하고 나올 때 signal(mutext)를 해야 한다. 

모니터에서 한 프로세스가 실행 중에 중단되었던 다른 프로세스가 재개하여 모니터에 들어오거나 인터럽트에 의해 일시중단 되는 경우를 대비해 next라는 세마포가 추가적으로 사용된다. 
또한 정수 변수 next_count도 일시중단된 프로세스 개수를 세기 위해 사용된다. 
```console
모니터 상호 배제를 보장한 실행
...
이후 x.signal()은 작은 우선순위를 가진 프로세스를 깨울 수 있다. 
...
## 9. 동기화 사례
Windows, Linux, Solaris, Pthreads API 등은 세마포, mutex, spinlocks 및 조건 변수 등을 제공한다. 

## 10. 대체 방안들

### 10.1. 트랜잭션 메모리(Transactional Memory)
