...
클라이언트 데이터 요청에 응답하는 데이터베이스를 실행하고 있는 서버, 
웹 브라우저 클라이언트에게 파일을 전달하는 웹 서버 등이 있다. 
...
하나의 CPU위에서 여러 운영체제가 작동할 수 있다.

## 클라우드 컴퓨팅
클라우드 컴퓨팅은 계산, 저장장치, 소프트웨어 서비스(스프레드시트 등), 플랫폼 서비스(데이터베이스 서버 등) 외에 여러 서비스를 네트워크를 통해 제공한다. 
EC2, GCP 등이 있다. 

## 실시간 내장형 시스템
자동차 엔진, 전자파 오븐 등 어느 곳에서나 볼 수 있다. 
특정한 작업만을 수행하며, 운영체제 없이 응용 전용 칩만으로 구동되는 시스템도 있다. 
내장형 시스템은 대부분 실시간성이 요구된다. 정해진 시간 제약 내에 올바른 결과를 내야 한다. 
